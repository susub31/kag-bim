...
data1<-data[,.(Week=Week+1,ClientID, ProductID, target, NewAverage)]
data=merge(data,data1[Week>8,.(targetl1=mean(target), NewAveragel1=mean(NewAverage)), by=.(Week,ClientID,ProductID)],all.x=T, by=c("Week","ClientID","ProductID"))
...
options(digits=18)
write.csv(results,file='results6.csv',row.names=F)
