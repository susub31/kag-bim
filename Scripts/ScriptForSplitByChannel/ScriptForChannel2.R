...
# training set - c2train
c2train=fread('c2train.csv', select=c("Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID", "Demand", "NumRecent2Weeks", "NumRecent3Weeks", "NumRecent4Weeks", "NumRecent5Weeks", "RecentWeek", "cum2WeekAverage", "cum3WeekAverage", "cum4WeekAverage", "AverageDemand", "WeekIntroduced", "OrderWeekRank"))

# testing set  - c2test
c2test=fread('c2test.csv',select = c("ID", "Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID"))


#get list of all products introduced after week #3; calculate average demand in the first week introduced
proddemand=filter(c2train, (Week==WeekIntroduced & Week==3))
newproddemand=filter(c2train, (Week==WeekIntroduced & Week>3))
...
ClientProdAverage = c2train[, .(ClientProdAvg=mean(Demand, na.rm=T)), by=c("ProductID","ClientID", "Route", "AgencyID")]
c2train = merge(c2train, ClientProdAverage, by=c("ProductID","ClientID", "Route", "AgencyID"), all.x=T)

c2train = merge(c2train, newproddemand, by=c("ProductID", "AgencyID"), all.x=T)
c2train = c2train[, cumAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage + AverageDemand)/4]
c2train = c2train[, cumRecentAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage)/3]
c2train = c2train[, NewCumAverage:=ifelse((NumRecent4Weeks >=2), cumRecentAverage, cumAverage)]
#RecentWeekData=filter(bimbotrain, OrderWeekRank==1)

train=c2train[Week>7]
test = c2test
rm(c2train)
rm(c2test)
...
write.csv(results,file='c2results1.csv',row.names=F)
