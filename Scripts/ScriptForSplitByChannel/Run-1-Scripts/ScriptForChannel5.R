...
ClientProdAverage = c5train[, .(ClientProdAvg=mean(Demand, na.rm=T)), by=c("ProductID","ClientID")]
c5train = merge(c5train, ClientProdAverage, by=c("ProductID","ClientID"), all.x=T)

c5train = merge(c5train, newproddemand, by="ProductID", all.x=T)
