...
proddemand=c4train[Week==WeekIntroduced & Week==3]
newproddemand=c4train[Week==WeekIntroduced & Week>3]
...
ClientProdAverage = c4train[, .(ClientProdAvg=mean(Demand, na.rm=T)), by=c("ProductID","ClientID")]
c4train = merge(c4train, ClientProdAverage, by=c("ProductID","ClientID"), all.x=T)

c4train = merge(c4train, newproddemand, by="ProductID", all.x=T)
...
c4train = c4train[, NewCumAverage:=ifelse((NumRecent4Weeks >=2), cumRecentAverage, (cumAverage + AverageDemand)/2 )]
