...
# training set - c8train
c8train=fread('c8train.csv', select=c("Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID", "Demand", "NumRecent2Weeks", "NumRecent3Weeks", "NumRecent4Weeks", "NumRecent5Weeks", "RecentWeek", "cum2WeekAverage", "cum3WeekAverage", "cum4WeekAverage", "AverageDemand", "WeekIntroduced", "OrderWeekRank"))

# testing set  - c8test
c8test=fread('c8test.csv',select = c("ID", "Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID"))


#get list of all products introduced after week #3; calculate average demand in the first week introduced
proddemand=c8train[Week==WeekIntroduced & Week==3]
newproddemand=c8train[Week==WeekIntroduced & Week>3]
...
ClientProdAverage = c8train[, .(ClientProdAvg=mean(Demand, na.rm=T)), by=c("ProductID","ClientID")]
c8train = merge(c8train, ClientProdAverage, by=c("ProductID","ClientID"), all.x=T)

c8train = merge(c8train, newproddemand, by="ProductID", all.x=T)
c8train = c8train[, cumAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage + AverageDemand)/4]
c8train = c8train[, cumRecentAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage)/3]
c8train = c8train[, NewCumAverage:=ifelse((NumRecent4Weeks >=2), cumRecentAverage, (cumAverage + AverageDemand)/2 )]
#RecentWeekData=filter(bimbotrain, OrderWeekRank==1)

train=c8train[Week>7]
test = c8test
rm(c8train)
rm(c8test)
...
write.csv(results,file='c8results1.csv',row.names=F)
