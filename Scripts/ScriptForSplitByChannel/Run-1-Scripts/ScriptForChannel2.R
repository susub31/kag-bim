...
proddemand=c2train[Week==WeekIntroduced & Week==3]
newproddemand=c2train[Week==WeekIntroduced & Week>3]
...
ClientProdAverage = c2train[, .(ClientProdAvg=mean(Demand, na.rm=T)), by=c("ProductID","ClientID")]
c2train = merge(c2train, ClientProdAverage, by=c("ProductID","ClientID"), all.x=T)

c2train = merge(c2train, newproddemand, by="ProductID", all.x=T)
...
c2train = c2train[, NewCumAverage:=ifelse((NumRecent4Weeks >=2), cumRecentAverage, (cumAverage + AverageDemand)/2 )]
