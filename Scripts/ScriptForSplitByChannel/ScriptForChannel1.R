...
proddemand=filter(c1train, (Week==WeekIntroduced & Week==3))
newproddemand=filter(c1train, (Week==WeekIntroduced & Week>3))
...
ClientProdRouteAvg = c1train[, .(ClientProdRouteAvg=mean(Demand, na.rm=T)), by=c("ProductID","ClientID", "Route", "AgencyID")]
c1train = merge(c1train, ClientProdRouteAvg, by=c("ProductID","ClientID", "Route", "AgencyID"), all.x=T)
...
c1train = merge(c1train, newproddemand, by=c("ProductID", "AgencyID"), all.x=T)
...
c1train = c1train[, NewCumAverage:=ifelse((NumRecent4Weeks >=2), cumRecentAverage, cumAverage )]
...
test$cumAverage=0
test$NewAverage=0
test$NewAverage=ifelse(test$cumAverage==0, ifelse((is.na(test$ClientProdRouteAvg) | test$ClientProdRouteAvg==0), ifelse((test$ClientProdAvg==0 | is.na(test$ClientProdAvg)), test$AvgProdDemand, test$ClientProdAvg), test$ClientProdRouteAvg), test$cumAverage)
