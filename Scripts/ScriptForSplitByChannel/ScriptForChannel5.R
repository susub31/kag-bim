...
c5train = c5train[, NewCumAverage:=ifelse((NumRecent4Weeks >=2), cumRecentAverage, cumAverage)]
...
features=names(data_train)[!(names(data_train) %in% c("ID","target","tst","AvgProdDemand","cumAverage", "NewAverage", "AverageDemand", "ClientProdAvg"))] 
rm(data)
