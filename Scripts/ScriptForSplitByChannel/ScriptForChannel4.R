...
# training set - c4train
c4train=fread('c4train.csv', select=c("Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID", "Demand", "NumRecent2Weeks", "NumRecent3Weeks", "NumRecent4Weeks", "NumRecent5Weeks", "RecentWeek", "cum2WeekAverage", "cum3WeekAverage", "cum4WeekAverage", "AverageDemand", "WeekIntroduced", "OrderWeekRank"))

# testing set  - c4test
c4test=fread('c4test.csv',select = c("ID", "Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID"))


#get list of all products introduced after week #3; calculate average demand in the first week introduced
proddemand=filter(c4train, (Week==WeekIntroduced & Week==3))
newproddemand=filter(c4train, (Week==WeekIntroduced & Week>3))
...
ClientProdAverage = c4train[, .(ClientProdAvg=mean(Demand, na.rm=T)), by=c("ProductID","ClientID", "Route", "AgencyID")]
c4train = merge(c4train, ClientProdAverage, by=c("ProductID","ClientID", "Route", "AgencyID"), all.x=T)

c4train = merge(c4train, newproddemand, by=c("ProductID", "AgencyID"), all.x=T)
c4train = c4train[, cumAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage + AverageDemand)/4]
c4train = c4train[, cumRecentAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage)/3]
c4train = c4train[, NewCumAverage:=ifelse((NumRecent4Weeks >=2), cumRecentAverage, cumAverage)]
#RecentWeekData=filter(bimbotrain, OrderWeekRank==1)

train=c4train[Week>7]
test = c4test
rm(c4train)
rm(c4test)
...
write.csv(results,file='c4results1.csv',row.names=F)
