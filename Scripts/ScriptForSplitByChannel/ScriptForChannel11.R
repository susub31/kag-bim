...
# training set - c11train
c11train=fread('c11train.csv', select=c("Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID", "Demand", "NumRecent2Weeks", "NumRecent3Weeks", "NumRecent4Weeks", "NumRecent5Weeks", "RecentWeek", "cum2WeekAverage", "cum3WeekAverage", "cum4WeekAverage", "AverageDemand", "WeekIntroduced", "OrderWeekRank"))

# testing set  - c11test
c11test=fread('c11test.csv',select = c("ID", "Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID"))


#get list of all products introduced after week #3; calculate average demand in the first week introduced
proddemand=c11train[Week==WeekIntroduced & Week==3]
newproddemand=c11train[Week==WeekIntroduced & Week>3]
...
ClientProdAverage = c11train[, .(ClientProdAvg=mean(Demand, na.rm=T)), by=c("ProductID","ClientID")]
c11train = merge(c11train, ClientProdAverage, by=c("ProductID","ClientID"), all.x=T)

c11train = merge(c11train, newproddemand, by="ProductID", all.x=T)
c11train = c11train[, cumAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage + AverageDemand)/4]
c11train = c11train[, cumRecentAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage)/3]
c11train = c11train[, NewCumAverage:=ifelse((NumRecent4Weeks >=2), cumRecentAverage, (cumAverage + AverageDemand)/2 )]
#RecentWeekData=filter(bimbotrain, OrderWeekRank==1)

train=c11train[Week>7]
test = c11test
rm(c11train)
rm(c11test)
...
write.csv(results,file='c11results1.csv',row.names=F)
