...
# training set - c6train
c6train=fread('c6train.csv', select=c("Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID", "Demand", "NumRecent2Weeks", "NumRecent3Weeks", "NumRecent4Weeks", "NumRecent5Weeks", "RecentWeek", "cum2WeekAverage", "cum3WeekAverage", "cum4WeekAverage", "AverageDemand", "WeekIntroduced", "OrderWeekRank"))

# testing set  - c6test
c6test=fread('c6test.csv',select = c("ID", "Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID"))


#get list of all products introduced after week #3; calculate average demand in the first week introduced
proddemand=c6train[Week==WeekIntroduced & Week==3]
newproddemand=c6train[Week==WeekIntroduced & Week>3]
...
ClientProdAverage = c6train[, .(ClientProdAvg=mean(Demand, na.rm=T)), by=c("ProductID","ClientID")]
c6train = merge(c6train, ClientProdAverage, by=c("ProductID","ClientID"), all.x=T)

c6train = merge(c6train, newproddemand, by="ProductID", all.x=T)
c6train = c6train[, cumAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage + AverageDemand)/4]
c6train = c6train[, cumRecentAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage)/3]
c6train = c6train[, NewCumAverage:=ifelse((NumRecent4Weeks >=2), cumRecentAverage, (cumAverage + AverageDemand)/2 )]
#RecentWeekData=filter(bimbotrain, OrderWeekRank==1)

train=c6train[Week>7]
test = c6test
rm(c6train)
rm(c6test)
...
write.csv(results,file='c6results1.csv',row.names=F)
