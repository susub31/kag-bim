...
# training set - c9train
c9train=fread('c9train.csv', select=c("Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID", "Demand", "NumRecent2Weeks", "NumRecent3Weeks", "NumRecent4Weeks", "NumRecent5Weeks", "RecentWeek", "cum2WeekAverage", "cum3WeekAverage", "cum4WeekAverage", "AverageDemand", "WeekIntroduced", "OrderWeekRank"))

# testing set  - c9test
c9test=fread('c9test.csv',select = c("ID", "Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID"))


#get list of all products introduced after week #3; calculate average demand in the first week introduced
proddemand=c9train[Week==WeekIntroduced & Week==3]
newproddemand=c9train[Week==WeekIntroduced & Week>3]
...
ClientProdAverage = c9train[, .(ClientProdAvg=mean(Demand, na.rm=T)), by=c("ProductID","ClientID")]
c9train = merge(c9train, ClientProdAverage, by=c("ProductID","ClientID"), all.x=T)

c9train = merge(c9train, newproddemand, by="ProductID", all.x=T)
c9train = c9train[, cumAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage + AverageDemand)/4]
c9train = c9train[, cumRecentAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage)/3]
c9train = c9train[, NewCumAverage:=ifelse((NumRecent4Weeks >=2), cumRecentAverage, (cumAverage + AverageDemand)/2 )]
#RecentWeekData=filter(bimbotrain, OrderWeekRank==1)

train=c9train[Week>7]
test = c9test
rm(c9train)
rm(c9test)
...
write.csv(results,file='c9results1.csv',row.names=F)
