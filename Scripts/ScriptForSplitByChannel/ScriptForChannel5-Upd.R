...
# training set - c5train
c5train=fread('c5train.csv', select=c("Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID", "Demand", "NumRecent2Weeks", "NumRecent3Weeks", "NumRecent4Weeks", "NumRecent5Weeks", "RecentWeek", "cum2WeekAverage", "cum3WeekAverage", "cum4WeekAverage", "AverageDemand", "WeekIntroduced", "OrderWeekRank"))

# testing set  - c5test
c5test=fread('c5test.csv',select = c("ID", "Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID"))


#get list of all products introduced after week #3; calculate average demand in the first week introduced
#proddemand=c5train[Week==WeekIntroduced & Week==3]
#newproddemand=c5train[Week==WeekIntroduced & Week>3]

proddemand=filter(c5train, (Week==WeekIntroduced & Week==3))
newproddemand=filter(c5train, (Week==WeekIntroduced & Week>3))

proddemand=proddemand[, .(AvgProdDemand= mean(Demand,na.rm=T)), by=c("ProductID", "AgencyID")]
newproddemand=newproddemand[, .(AvgProdDemand= mean(Demand,na.rm=T)), by=c("ProductID","AgencyID")]

newproddemand=rbind(proddemand, newproddemand)

newproddemand = newproddemand %>%
  group_by(ProductID, AgencyID) %>%
  summarise(AvgProdDemand=mean(AvgProdDemand))
rm(proddemand)

ClientProdAverage = c5train[, .(ClientProdAvg=mean(Demand, na.rm=T)), by=c("ProductID","ClientID", "Route", "AgencyID")]
c5train = merge(c5train, ClientProdAverage, by=c("ProductID","ClientID", "Route", "AgencyID"), all.x=T)

c5train = merge(c5train, newproddemand, by=c("ProductID", "AgencyID"), all.x=T)
c5train = c5train[, cumAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage + AverageDemand)/4]
c5train = c5train[, cumRecentAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage)/3]
c5train = c5train[, NewCumAverage:=ifelse((NumRecent4Weeks >=2), cumRecentAverage, (cumAverage + AverageDemand)/2 )]
#RecentWeekData=filter(bimbotrain, OrderWeekRank==1)

train=c5train[Week>7]
test = c5test
rm(c5train)
rm(c5test)
...
test =  merge(test,  newproddemand, by=c("ProductID", "AgencyID"), all.x=TRUE)

# merge details from ClientProdAverage will have only averages for clients/products in training set
test = merge(test, ClientProdAverage, by=c("ProductID","ClientID", "Route", "AgencyID"), all.x=T)

train$NewAverage=0
train$NewAverage=ifelse(train$NewCumAverage==0, train$ClientProdAvg==0, train$NewCumAverage)
...
#test$NewAverage=ifelse(test$cumAverage==0, test$ClientProdAvg, test$cumAverage)
...
write.csv(results,file='c5results1.csv',row.names=F)
