...
# training set - c7train
c7train=fread('c7train.csv', select=c("Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID", "Demand", "NumRecent2Weeks", "NumRecent3Weeks", "NumRecent4Weeks", "NumRecent5Weeks", "RecentWeek", "cum2WeekAverage", "cum3WeekAverage", "cum4WeekAverage", "AverageDemand", "WeekIntroduced", "OrderWeekRank"))

# testing set  - c7test
c7test=fread('c7test.csv',select = c("ID", "Week", "AgencyID", "ChannelID", "Route", "ClientID", "ProductID"))


#get list of all products introduced after week #3; calculate average demand in the first week introduced
proddemand=c7train[Week==WeekIntroduced & Week==3]
newproddemand=c7train[Week==WeekIntroduced & Week>3]
...
ClientProdAverage = c7train[, .(ClientProdAvg=mean(Demand, na.rm=T)), by=c("ProductID","ClientID")]
c7train = merge(c7train, ClientProdAverage, by=c("ProductID","ClientID"), all.x=T)

c7train = merge(c7train, newproddemand, by="ProductID", all.x=T)
c7train = c7train[, cumAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage + AverageDemand)/4]
c7train = c7train[, cumRecentAverage:=(cum2WeekAverage + cum3WeekAverage + cum4WeekAverage)/3]
c7train = c7train[, NewCumAverage:=ifelse((NumRecent4Weeks >=2), cumRecentAverage, (cumAverage + AverageDemand)/2 )]
#RecentWeekData=filter(bimbotrain, OrderWeekRank==1)

train=c7train[Week>7]
test = c7test
rm(c7train)
rm(c7test)
...
write.csv(results,file='c7results1.csv',row.names=F)
