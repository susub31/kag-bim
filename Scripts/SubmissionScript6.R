...
# capture the recent 2 weeks of data for Client/Product; make them the data for Weeks #8 and #9 so that we can predict for #10 and #11
RecentWeekData=filter(bimbotrain, OrderWeekRank<=2)
RecentWeekData$Week=ifelse(RecentWeekData$OrderWeekRank==1, 9, 8)
...
# dataset with all products in 'test' dataset; newproddemand will have only averages for products in training set
test = merge(test, newproddemand, by="ProductID", all.x=TRUE)

# dataset with products that have entries in training set
testintrain=merge(test, newproddemand, by="ProductID")
...
write.csv(results,file='results9.csv',row.names=F)
